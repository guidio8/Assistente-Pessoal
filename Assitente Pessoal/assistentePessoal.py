...
def run_alexa():
    ...
    if language == 1:
        if 'play' in command:
            ...
            if 'lucas' in command:
                threadNova = threading.Thread(target=mandar_mensagem_whats,
                                              args=("HERE GOES WHATSAPP NUMBER", mensagem))
                threadNova.start()
            if 'leonardo' in command:
                threadNova = threading.Thread(target=mandar_mensagem_whats,
                                              args=("NÃO DEIXAR OS NÚMEROS NA HORA DE PUBLICAR", mensagem))
                threadNova.start()
